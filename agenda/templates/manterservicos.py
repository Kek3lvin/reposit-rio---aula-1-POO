...
class  ManterServicoUI:
 ...
 def inserir(): 
  des = st.text_input("Descrição do serviço:")
  valor = st.number_input("Informe o valor:")
  if st.button("Inserir"):
    View.servico_inserir(des, valor)
 ...
 def atualizar():
   servicos = View.servico_listar()
   if len(servicos) == 0: st.write("Nenhum serviço foi cadastrado")
   else:
     op = st.selectbox("Atualização dos serviços", servicos)
     des = st.text_input("Nova descrição", op.get_des())
     valor = st.number_input("Novo valor", op.get_valor())
     if st.button("Atualizar"):
       id = op.get_id()
       View.servico_atualizar(id, des, valor) 
